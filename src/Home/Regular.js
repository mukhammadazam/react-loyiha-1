import png1 from './imges/regular1.png'
import png2 from './imges/regular2.png'
import png3 from './imges/regular3.png'
import png4 from './imges/regular4.png'
import png5 from './imges/regular5.png'
import png6 from './imges/regular6.png'
import png7 from './imges/regular7.png'
import png8 from './imges/regular8.png'
import png9 from './imges/regular9.png'
import png10 from './imges/regular10.png'
import png11 from './imges/regular11.png'
import png12 from './imges/regular12.png'
function BodyOnlyExample() {
    return (
        <div className="regular-site">
            <div className="container">
                <div className="regular">
                    <h2 className="regular__title">Our Regular Menu Pack</h2>
                    <ul className="regular__list d-flex align-items-center justify-content-between flex-wrap gap-4">
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">All</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">Shawarma</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">Turk Kebab</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">Hamburger Kebab</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">Doner kebab</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">Shish kebab</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">French fries Pizza</li>
                        <li className="regular__list--item px-4 py-2 border border-danger border-2 rounded-pill">Desserts</li>
                    </ul>
                    <div className="regular__minu gap-3 justify-content-lg-between justify-content-center justify-content-sm-center  row">
                        <div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png1} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div>
                        <div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png2} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div>
                        <div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png3} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png4} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png5} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png6} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png7} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png8} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png9} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png10} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png11} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div><div className="regular__minu--item col-12 col-sm-6 col-lg-3">
                            <div className='d-flex align-items-center justify-content-center'><img src={png12} /></div>
                            <p className='text-center'>Barbecue Shish kebab Shashlik Skewer</p>
                            <p className='text-center regular__minu--text text-danger'>$12.00</p>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default BodyOnlyExample;

const list = document.querySelectorAll('.regular__list--item');
console.log(list);

